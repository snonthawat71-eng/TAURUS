...
export function HKMapViewer({ onClose, onResult }: { onClose: () => void; onResult?: (t: Transit) => void }) {
  const [zoom, setZoom] = useState(1)
  const [from, setFrom] = useState<string | null>(null)
  const [to, setTo] = useState<string | null>(null)
  const [q, setQ] = useState('')

  const transit = useMemo(() => (from && to ? computeRouteHK(from, to) : null), [from, to])
  ...
  const matches = q.trim()
    ? HK_STATION_LIST.filter((s) => s.toLowerCase().includes(q.trim().toLowerCase())).slice(0, 8)
    : []
    ...
    <div className="fixed inset-0 z-[120] bg-canvas flex flex-col">
      ...
      <div className="px-4 py-2.5 shrink-0 relative">
        <div className="flex items-center gap-2 rounded-md hairline px-3 h-10 bg-surface">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ค้นหาชื่อสถานี (เช่น Central, Mong Kok)"
          ...
          <div className="absolute left-4 right-4 mt-1 card p-1 shadow-lg z-20 max-h-60 overflow-y-auto">
            {matches.map((s) => (
              <button key={s} onClick={() => { pick(s); setQ('') }}
                ...
                {HK_NETWORK.filter((l) => l.stations.includes(s)).map((l) => <span key={l.id} className="size-2.5 rounded-full" style={{ background: l.color }} />)}
