...
import { SZ_NETWORK, SZ_STATION_LIST, computeRouteShenzhen } from '@/lib/metro/shenzhenNetwork'
import { SZ_LEGEND } from '@/lib/metro/shenzhenGeo'
import type { Transit } from '@/lib/database.types'

export function ShenzhenMapViewer({ onClose, onResult }: { onClose: () => void; onResult?: (t: Transit) => void }) {
  const [zoom, setZoom] = useState(1)
  const [from, setFrom] = useState<string | null>(null)
  const [to, setTo] = useState<string | null>(null)
  const [q, setQ] = useState('')

  const transit = useMemo(() => (from && to ? computeRouteShenzhen(from, to) : null), [from, to])
  ...
  const matches = q.trim()
    ? SZ_STATION_LIST.filter((s) => s.toLowerCase().includes(q.trim().toLowerCase())).slice(0, 8)
    : []
  ...
  return createPortal(
    <div className="fixed inset-0 z-[705] bg-canvas flex flex-col">
      <div className="flex items-center justify-between px-4 h-14 shrink-0" style={{ borderBottom: '0.5px solid var(--color-line)' }}>
        <div className="text-[15px] font-medium">รถไฟฟ้าเซินเจิ้น · เลือกสถานี</div>
      ...
      <div className="px-4 py-2.5 shrink-0 relative">
        <div className="flex items-center gap-2 rounded-md hairline px-3 h-10 bg-surface">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ค้นหาชื่อสถานี (เช่น Luohu, Futian, Window of the World)"
          ...
          <div className="absolute left-4 right-4 mt-1 card p-1 shadow-lg z-20 max-h-60 overflow-y-auto">
            {matches.map((s) => (
              <button key={s} onClick={() => { pick(s); setQ('') }}
                ...
                {SZ_NETWORK.filter((l) => l.stations.includes(s)).map((l) => <span key={l.id} className="size-2.5 rounded-full" style={{ background: l.color }} />)}
      ...
      <div className="flex-1 relative min-h-0">
        <PinchZoomPane zoom={zoom} setZoom={(z) => setZoom(z)} min={0.6} max={4} className="absolute inset-0 overflow-auto bg-surface-2/40">
          <ShenzhenMetroMap zoom={zoom} from={from} to={to} onTap={pick} />
        </PinchZoomPane>
      ...
      <div className="shrink-0 p-4 bg-surface" style={{ borderTop: '0.5px solid var(--color-line)', maxHeight: '45dvh', overflowY: 'auto' }}>
        ...
        {transit && transit.legs.length > 0 ? (
          ...
          <p className="text-[12px] text-ink-3 mt-2">แตะสถานีบนแผนที่ (จุดที่แตะได้) หรือค้นหาชื่อ เลือกต้นทาง + ปลายทาง ระบบจะคำนวณจุดเปลี่ยนสายให้ · <span className="text-ink-3">บางสถานีย่อยให้ใช้ช่องค้นหา</span></p>
        )}
        {/* legend */}
        <div className="flex flex-wrap gap-x-3 gap-y-1 mt-3 pt-3" style={{ borderTop: '0.5px solid var(--color-line)' }}>
          {SZ_LEGEND.map((l) => (
            <span key={l.name} className="inline-flex items-center gap-1 text-[10px] text-ink-3">
