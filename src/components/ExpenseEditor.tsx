...
import { IconTrash, IconPaperclip, IconLoader2, IconCheck, IconReceipt2, IconEye, IconScan, IconChevronDown } from '@tabler/icons-react'
...
// Same look as the place/hotel editors: one bordered card. Every section gets
// the SAME small label + consistent spacing, so the form reads as a tidy list.
  ...
  return (
    <Drawer open={open} onClose={onClose} title={initial ? 'แก้ไขค่าใช้จ่าย' : 'เพิ่มค่าใช้จ่าย'}>
      <div className="space-y-3">
        <div className="rounded-[13px] bg-surface p-3.5 space-y-4" style={{ border: '0.5px solid var(--color-line)' }}>
          {/* 1) ชื่อรายการ */}
          <Section label="ชื่อรายการ">
            <div className="relative">
              <LeadIcon><IconReceipt2 size={15} /></LeadIcon>
              <input className={iconField} value={name} onChange={(e) => setName(e.target.value)} placeholder="เช่น มื้อค่ำ Haidilao" />
            </div>
          </Section>

          {/* 2) ประเภท — a uniform 3-column grid: every pill the same size, two
                tidy rows (a ragged wrap looked messy) */}
          ...
          {/* 4) ราคา + สกุลเงิน — no in-field symbol (it collided with the
                placeholder); the currency picker says it all */}
          <Section label="ราคา">
            <div className="flex items-stretch gap-2">
              <input type="number" inputMode="decimal"
                className="hairline rounded-[9px] text-[17px] font-semibold tabular-nums h-11 px-3.5 bg-surface flex-1 min-w-0 outline-none focus:border-brand"
              ...
              <div className="relative shrink-0">
                <select value={currency} onChange={(e) => setCurrency(e.target.value)} aria-label="สกุลเงิน"
                  className="hairline rounded-[9px] h-11 bg-surface text-[13px] font-medium pl-3 pr-8 outline-none focus:border-brand appearance-none">
          ...
          {/* 5) หารกับใคร */}
          <Section label={`หารกับใคร · ${split.length} คน`}
                ...
                return (
                  <button key={t.id} onClick={() => toggleSplit(t.id)}
                    ...
                    style={on
                      ? { background: 'var(--color-brand)', color: '#fff' }
                      : { background: 'var(--color-surface-2)', color: 'var(--color-ink-2)' }}>
                    {on && <IconCheck size={13} />} {t.nickname}
                  </button>
                )
              })}
            </div>
          ...
          <Section label="สลิป / ใบเสร็จ"
            right={receipt ? <button onClick={viewSlip} className="text-[11px] text-brand-mid font-medium inline-flex items-center gap-1"><IconEye size={13} /> ดูสลิป</button> : undefined}>
            <div className="grid grid-cols-2 gap-1.5">
              <button onClick={() => slipInput.current?.click()} disabled={uploading || scanning}
                className="h-10 rounded-[9px] inline-flex items-center justify-center gap-1.5 text-[12.5px] font-medium text-ink-2 disabled:opacity-50"
              ...
              <button onClick={() => scanInput.current?.click()} disabled={uploading || scanning}
                className="h-10 rounded-[9px] inline-flex items-center justify-center gap-1.5 text-[12.5px] font-semibold disabled:opacity-60"
            ...
            {scanning && <p className="text-[10.5px] text-ink-3 mt-1.5">อ่านตัวเลขจากรูปบนเครื่องของคุณ (ครั้งแรกอาจใช้เวลาสักครู่)</p>}
