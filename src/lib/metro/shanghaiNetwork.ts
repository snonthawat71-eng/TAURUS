// Shanghai Metro network for routing — built from the line/station data in
// shanghai.ts (same source the manual suggestions use). Mirrors hkNetwork.ts:
// a Dijkstra over (station, line) states that penalises transfers, returning
// a Transit broken into per-line legs.
import { SHANGHAI } from './shanghai'
import type { Transit, TransitLeg } from '@/lib/database.types'

interface SHLine { id: string; name: string; color: string; stations: string[] }

// Give every line a stable id (its name) and keep the official colour.
export const SH_NETWORK: SHLine[] = SHANGHAI.lines.map((l) => ({
  id: l.name, name: l.name, color: l.color, stations: l.stations,
}))

export const lineColor = (id: string) => SH_NETWORK.find((l) => l.id === id)?.color ?? '#666'
...
export const SH_STATION_LIST = Object.keys(stationLines).sort()
export const isSHStation = (name: string) => !!stationLines[name]
export const linesOf = (name: string) => Array.from(stationLines[name] ?? [])

function idxOnLine(line: string, station: string) {
  return SH_NETWORK.find((x) => x.id === line)!.stations.indexOf(station)
}

export function computeRouteShanghai(fromName: string, toName: string): Transit | null {
  ...
  for (let i = 1; i <= path.length; i++) {
    if (i === path.length || path[i].l !== path[seg].l) {
      ...
      const l = SH_NETWORK.find((x) => x.id === line)!
