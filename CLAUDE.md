...
- **Always test before deploy.** Before any deploy, run `npm run lint` then `npm run build` and only deploy if both pass. If either fails, stop and report to the user — never deploy over a broken build.
